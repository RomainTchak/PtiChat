                ...
                if (messageRecu.Sender == "Server")
                {
                    ...
                    foreach(string user in ExchangedMessages.Keys)
                    {
                        if (!ConnectedUsers.Contains(user))
                        {
                            ...
                            {
                                ConversationList.Add($"{messageToStore.SendTime},{messageToStore.Sender},{messageToStore.Target},{messageToStore.Body}");
                            }
                    ...
                    foreach (string user in connectedPeople)
                    {
                        ...
                        if (!(ExchangedMessages.ContainsKey(user)))
                        {
                            ...
                            foreach(string message in messageHistory)
                            {
                                ...
                                storedMessage.SendTime= Attributes[0];
                                storedMessage.Sender = Attributes[1];
                                storedMessage.Target = Attributes[2];
                                storedMessage.Body = Attributes[3];
                                ExchangedMessages[user].Add(storedMessage);
                                
                            }
                            //A la connection de chaque nouveau client on charge notre hitorique de discussion avec lui dans
                            //ExchangedMessages
